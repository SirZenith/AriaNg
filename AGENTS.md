...
src/
  ...
  components/                  # 通用组件（AppLayout、PanelBar、TopBarTabs、工具栏、settings/ 通用设置组件、PieceBar/PieceMap 等）
...
- **全局布局**：`src/components/AppLayout.tsx` 提供主滚动容器 `<main data-scroll-container>`、顶/底栏插槽（`PanelBarProvider` + `BarHost`）与通知容器。页面用 `<TopBar>`/`<BottomBar>` 声明栏内容，内容会 portal 到布局层（React 树仍在页面内，页面局部状态可直接驱动栏内容）；分裂底栏（左侧玻璃胶囊组 + 右侧独立块，分别靠屏幕两端，iOS 26 风格）用 `SplitBottomBar`（内部包 `BottomBar`，样式类为 `styles/components.css` 的 `.bottom-bar-split`/`.bottom-bar-group`/`.bottom-bar-item`），新建任务页底栏即用它；单组居中底栏（如确认按钮）用 `CenteredBottomBar`（同套玻璃样式），新建任务设置页底栏即用它；顶/底栏槽位均无 fallback（`BottomNav` 组件目前未被渲染，保留备用）。顶栏内容示例：主页 → 内嵌 `Home` 标题栏、`TaskListPage` → 返回标题 + 任务列表页签（下载中/等待/已停止）、`TaskDetailPage` → `TaskDetailToolbar`、设置页 → `SettingsPage`（内部渲染 `SettingsToolbar`）；任务列表页底栏由 `TaskListToolbar` 渲染（左组为新建/开始/暂停/删除/全选按钮，右侧搜索钮点击后在底栏上方淡入搜索输入框，失焦且为空时淡出、有内容时保持），任务详情页底栏用 `SplitBottomBar`（左组为开始/暂停、重试、复制下载链接，右侧为删除）。顶栏页签列表统一由 `TopBarTabs` 渲染（`tabs` + `activeKey`，单项支持 `to` 链接或 `onClick` 按钮、`icon`、`count` 徽标，`hideLabelsOnMobile` 控制移动端仅显示图标），任务列表、任务详情、新建任务页均用它。
