...
- **路由**：`src/App.tsx` 集中定义（HashRouter）。默认入口为主页 `/home`：未连接 RPC 时展示 RPC 服务器配置列表（复用 `RpcSettingsMenu`），已连接时展示连接情况与功能入口。AriaNg 自身设置使用 `/ariang` 路由族（`/ariang/general[|:item]`、`/ariang/rpc[|:item[|:field]]`、`/ariang/importExport`），入口内嵌在主页的“AriaNg Settings”分区；`/ariang` 与未匹配的 `/ariang/*` 重定向 `/home`。aria2 设置前缀为 `/settings`，入口内嵌在主页的“Settings”分区（`SettingsMenu`），每个路由对应 `features/settings/` 的独立页面（`BasicSettingsPage`/`ProtocolSettingsPage`/`RpcGlobalSettingsPage`/`StatusPage` 等），共用 `SettingsPage` 外壳；`/settings` 列表页已移除，访问时重定向 `/home`，直接子界面（`/settings/basic` 等）的顶栏返回按钮也返回 `/home`，深层页面返回各自父级；协议分类直链（如 `/settings/bt`）重定向到 `/settings/protocol`，该页把所有协议分类合并为 `SettingsSection` 列表。兼容旧式 `#!/` 路径与 `#!/new/:url`、`#!/settings/rpc/set/...` 命令行链接。
- **全局布局**：`src/components/AppLayout.tsx` 提供主滚动容器 `<main data-scroll-container>`、顶/底栏插槽（`PanelBarProvider` + `BarHost`）与通知容器。页面用 `<TopBar>`/`<BottomBar>` 声明栏内容，内容会 portal 到布局层（React 树仍在页面内，页面局部状态可直接驱动栏内容）；顶/底栏槽位均无 fallback（`BottomNav` 组件目前未被渲染，保留备用）。顶栏内容示例：主页 → 内嵌 `Home` 标题栏、`TaskListPage` → `TaskListToolbar`、`TaskDetailPage` → `TaskDetailToolbar`、设置页 → `SettingsPage`（内部渲染 `SettingsToolbar`）；任务详情页底栏提供开始/暂停、重试、删除操作。
